...
def handler(event, context):
    ...
    # Ensure requests arrive via CloudFront — direct Lambda URL calls are rejected.
    # Full auth (Cognito JWT) is enforced upstream by the Lambda@Edge origin-request function.
    via_ok = 'via' in headers and 'cloudfront.net' in via
    if not via_ok:
        _log(request_id, 'warning', 'Request denied',
             reason='not via CloudFront', client_ip=client_ip)
        return {
            'statusCode': '403',
